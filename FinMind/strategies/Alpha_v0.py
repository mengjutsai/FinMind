...
class Alpha_v1(Strategy):
    ...
    def _plot_all(self, stock_id, stock_name, start_date, end_date):
        """一次性繪出所有日圖"""
    ...
    def create_trade_sign(self, stock_price, **kwargs):
        ...
        image_dir, stock_dir = self._plot_all(stock_id, stock_name,
                                              sp["date"].min().strftime("%Y-%m-%d"),
                                              sp["date"].max().strftime("%Y-%m-%d"))

        df_det = run_inference(
            model_path=self.model_path,
            source_dir=image_dir,
            save_dir=image_dir,      # 可忽略或同一路徑
            output_dir=os.path.join(stock_dir, "image-seg"),
        ...
        for i, row in sp.iterrows():
            ...
            for idx, trade in enumerate(active_trades):
                if low <= trade["stop_price"]:
                    ...
                    sp.loc[i, "signal"] -= trade["position"]
                    to_remove.append(idx)
                    records.append({"date": date, "action": "STOP", "price": exit_price})
                elif high >= trade["target1"]:
                    exit_price = trade["target1"]
                    sp.loc[i, "signal"] -= trade["position"]
                    to_remove.append(idx)
                    records.append({"date": date, "action": "TP1", "price": exit_price})
            for idx in sorted(to_remove, reverse=True):
                active_trades.pop(idx)

            # 入場邏輯
            det_today = df_det[df_det["date_ref"] == date.strftime("%Y-%m-%d")]
            det_today = det_today[det_today["label"].isin(self.long_labels)]
            if not det_today.empty:
                best = det_today.loc[det_today["price_1st"].idxmax()]
                ...
                trade = dict(entry_date=date, entry_price=price,
                             target1=p1, target2=p2,
                             stop_price=stop_tmp, position=add_units)
                active_trades.append(trade)
                sp.loc[i, "signal"] = add_units
                records.append({"date": date, "action": "BUY", "price": price})

            sp.loc[i, "position"] = sum(t["position"] for t in active_trades)

        out_log = os.path.join(stock_dir, f"trade_log_{stock_id}.csv")
