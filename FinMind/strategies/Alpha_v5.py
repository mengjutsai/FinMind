...
class FactorEngine:
    ...
    # -------------------------
    # Broker (沿用你 V11 的權重架構)
    # broker_ratio + zscore 組合成 0~1 分數
    # -------------------------
    def broker_factor(self, broker_ratio, zscore):
        ...
        # 權重方式沿用 V11 的計算邏輯，轉換成 0~1 分數
        ratio_score = min(1.0, broker_ratio / 0.20)       # 0.05 達門檻，0.20 將滿分
        zscore_score = min(1.0, zscore / 5.0)             # 2 達門檻，5 達滿分
    ...
    # 總因子
    # -------------------------
    def total_factor(self, f_pattern, f_broker):
    ...
    def create_trade_sign(self, stock_price: pd.DataFrame, **kwargs):
        ...
        for i, row in sp.iterrows():
            ...
            if not df_today.empty:
                ...
                total_f = self.engine.total_factor(f_pattern, f_broker)
